/* Translation + Speech Recognition module
 * Exposes:
 *  - getTranslation(text[, endpoint]) -> Promise resolving to {untranslated, translated} or {error}
 *  - createRecognizer(options) -> { start(), stop(), isListening, destroy() }
 *
 * This file has no DOM dependencies. It attaches to window.TranslatorModule and supports CommonJS.
 */
(function () {
    const defaultConfig = {
    ...
    function getTranslation(text, endpointOrCallback, maybeCallback) {
        // Support both getTranslation(text, callback) and getTranslation(text, endpoint)
